      ...
      <div className="portfolio-details">
        ...
        <div className="port-gallery">
          ...
          <div className="port-images">
              ...
              <img className="thumb" src={portfolio.img2 ? portfolio.img2 : portfolio.img} />
           
          </div>
          <div className="port-images">
           
              <img className="thumb" src={ portfolio.img3 ? portfolio.img3 : portfolio.img } />
           
          </div>
        </div>
        <div className="port-content px-5">
        <h1 className="text-center"><a href={portfolio.siteLink}>Live Preview {portfolio.name}</a></h1>
        <br /><br /><br />
        <h4 className="m-2">Description</h4>
        {ReactHtmlParser(portfolio.description)}
        <br /><br />
        <h4 className="m-2">Feature</h4>
        <p><FontAwesomeIcon className="text-info fs-6"  icon={faCheckDouble} /> {portfolio.features?.split('.')[0]}</p>
        <p><FontAwesomeIcon className="text-info fs-6"  icon={faCheckDouble} /> {portfolio.features?.split('.')[1]}</p>
        </div>
      </div>
